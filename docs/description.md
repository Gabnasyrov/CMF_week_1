...
| `data/bybit_trades/<sym>.parquet` | `timestamp, ticker, side, price, amount` |
| `data/bybit_booktickers/<sym>.parquet` | `timestamp, ticker, bid_price, bid_amount, ask_price, ask_amount` |

`ticker` равен `perp:btcusdt` / `perp:ethusdt` для Binance и `btcusdt` / `ethusdt` для Bybit (в обоих случаях совпадает с именем файла).

Сборка нативных Bybit trades/BBO (не трогает Binance):

```bash
cd analysis
python3 build_bybit_market_data.py --symbols btcusdt --start-date 2025-12-01 --end-date 2026-02-28
python3 validate_bybit_market.py
```
