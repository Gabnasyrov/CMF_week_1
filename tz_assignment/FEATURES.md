...
Все колонки строятся в `research/strategies_bundle/lib/feature_builder.py` из `data/enriched/` (см. `build_features.py` / `analysis/build_features.py`).
...
## Enriched inputs (`build_features.py`)

| Поток | Колонки в parquet |
|-------|-------------------|
| Binance BBO | `mid`, `spread`, `spread_bps`, `book_imbalance`, `microprice`, `microprice_g`, `ofi_increment`, `ofi_cumulative` |
| Binance trades | asof BBO state + `trade_flow_1s`, `trade_flow_5s`, OFI 1s/5s |
| Binance / Bybit liq | `order_flow_imbalance`, 1s bars, cumulative (Bybit: +200 ms в панели стратегий) |
