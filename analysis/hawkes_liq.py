...
def hawkes_loglik(events: np.ndarray, mu: float, alpha: float, kappa: float) -> float:
    """Events in seconds, sorted. λ(t)=μ+α Σ exp(-κ(t-t_j))."""
    if len(events) < 3:
        return -1e12
    ...
    for i, ti in enumerate(t):
        ...
        ll += np.log(lam)
        s += 1.0
    comp += mu * max(1e-6, 1e-3)  # tiny tail
    return float(ll - comp)


def hawkes_intensity_at(events: np.ndarray, t_sec: float, mu: float, alpha: float, kappa: float) -> float:
    past = events[events < t_sec]
    if len(past) == 0:
        return mu
    dt = t_sec - past
    return float(mu + alpha * np.exp(-kappa * dt).sum())


def fit_hawkes_mle(events_us: np.ndarray, max_events: int = 50_000) -> dict:
    """Fit (μ, α, κ) on liquidation timestamps (microseconds → seconds)."""
    if len(events_us) < 50:
